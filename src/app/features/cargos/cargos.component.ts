  ...
  template: `
    <div class="page">
      <div class="page-header">
        <div><h2>💼 Cargos</h2><p class="sub">Gestión de cargos laborales</p></div>
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr><th>Código</th><th>Nombre del Cargo</th><th>Acciones</th></tr>
          </thead>
          <tbody>
            @for (c of items(); track c.codCargo) {
              <tr>
                <td><code>{{ c.codCargo }}</code></td>
                <td>{{ c.nombreCargo }}</td>
                <td class="actions">
                  <button class="btn-edit" (click)="editItem(c)">✏️</button>
                  <button class="btn-del" (click)="deleteItem(c.codCargo)">🗑️</button>
                </td>
              </tr>
            } @empty {
              <tr><td colspan="3" class="empty">No hay cargos registrados</td></tr>
            }
          </tbody>
        </table>
      </div>
        ...
        <div class="modal-overlay" (click)="closeForm()">
          <div class="modal" (click)="$event.stopPropagation()">
            <div class="modal-header">
              <h3>{{ editingId() ? 'Editar' : 'Nuevo' }} Cargo</h3>
              <button class="close-btn" (click)="closeForm()">✕</button>
            </div>
            <form [formGroup]="form" (ngSubmit)="onSubmit()" class="modal-body">
              @if (!editingId()) {
                <div class="field">
                  <label>Código Cargo (3 chars) *</label>
                  <input formControlName="codCargo" maxlength="3" placeholder="GER">
                  @if (form.get('codCargo')?.invalid && form.get('codCargo')?.touched) {
                    <span class="err">Código requerido (máx 3 chars)</span>
                  }
                </div>
              }
              <div class="field">
                <label>Nombre del Cargo *</label>
                <input formControlName="nombreCargo" placeholder="Ej: Gerente de Ventas">
                @if (form.get('nombreCargo')?.invalid && form.get('nombreCargo')?.touched) {
                  <span class="err">Nombre requerido (mín 2 chars)</span>
                }
              </div>
...
export class CargosComponent implements OnInit {
  ...
  constructor(private svc: CargoService, private fb: FormBuilder) {
    this.form = this.fb.group({
      codCargo:    ['', [Validators.required, Validators.maxLength(3)]],
  ...
  loadData() {
    this.svc.getAll().subscribe(r => this.items.set(r.data ?? []));
  }

  openForm() {
    this.form.reset();
    this.editingId.set(null);
    this.form.get('codCargo')?.enable();
    this.showForm.set(true);
  }

  editItem(c: Cargo) {
    this.editingId.set(c.codCargo);
    this.form.patchValue(c);
    this.form.get('codCargo')?.disable();
    this.showForm.set(true);
  }
  ...
  onSubmit() {
    ...
    obs.subscribe({
      next: () => {
        this.showAlert(`Cargo ${this.editingId() ? 'actualizado' : 'creado'} exitosamente`, 'success');
  ...
  deleteItem(id: string) {
    if (!confirm(`¿Eliminar cargo ${id}?`)) return;
    this.svc.delete(id).subscribe({
      next: () => { this.showAlert('Cargo eliminado', 'success'); this.loadData(); },
      error: e => this.showAlert(e.message, 'error')
    });
  }
